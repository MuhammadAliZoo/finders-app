...
const getMetrics = async (startDate) => {
  ...
  const activeUsersChange =
    previousActiveUsers === 0 ? 100 : Math.round(((activeUsers - previousActiveUsers) / previousActiveUsers) * 100)
  ...
  const newItemsChange =
    previousNewItems === 0 ? 100 : Math.round(((newItems - previousNewItems) / previousNewItems) * 100)
  ...
  const matchesChange = previousMatches === 0 ? 100 : Math.round(((matches - previousMatches) / previousMatches) * 100)
  ...
  const disputesChange =
    previousDisputes === 0 ? 0 : Math.round(((disputes - previousDisputes) / previousDisputes) * 100)
...
const getPendingApprovals = async () => {
  const pendingItems = await Item.find({ status: "pending" }).sort({ createdAt: 1 }).limit(5).lean()
...
export const getAIRecommendation = asyncHandler(async (req, res) => {
  ...
  const recommendation = {
    ...
    additionalNotes: "Consider requesting additional verification due to the high value of the item.",
  }
